...
class mod_ouilforum_external extends external_api {
    ...
    public static function get_forums_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $forums = get_all_instances_in_courses('ouilforum', $courses);
            foreach ($forums as $forum) {

                $course = $courses[$forum->course];
                $cm = get_coursemodule_from_instance('ouilforum', $forum->id, $course->id);
                ...
                list($forum->intro, $forum->introformat) = external_format_text($forum->intro, $forum->introformat,
                                                                                $context->id, 'mod_ouilforum', 'intro', 0);
                // Discussions count. This function does static request cache.
                $forum->numdiscussions = ouilforum_count_discussions($forum, $cm, $course);
                $forum->cmid = $forum->coursemodule;
                $forum->cancreatediscussions = ouilforum_user_can_post_discussion($forum, null, -1, $cm, $context);
    ...
    public static function get_forum_discussions($forumids, $limitfrom = 0, $limitnum = 0) {
        ...
        foreach ($forumids as $id) {
            ...
            require_capability('mod/ouilforum:viewdiscussion', $modcontext);
            ...
            if ($discussions = $DB->get_records_select('ouilforum_discussions', $select, $params, 'timemodified DESC', '*',
                                                            $limitfrom, $limitnum)) {
                ...
                if ($cantrack = ouilforum_tp_can_track_forums($forum)) {
                    if ($forumtracked = ouilforum_tp_is_tracked($forum)) {
                        $unreads = ouilforum_get_discussions_unread($cm);
                    }
                }
                // The forum function returns the replies for all the discussions in a given forum.
                $replies = ouilforum_count_discussion_replies($id);

                foreach ($discussions as $discussion) {
                    // This function checks capabilities, timed discussions, groups and qanda forums posting.
                    if (!ouilforum_user_can_see_discussion($forum, $discussion, $modcontext)) {
                        continue;
                    }
                    ...
                    // Get the subject.
                    $subject = $DB->get_field('ouilforum_posts', 'subject', array('id' => $discussion->firstpost), MUST_EXIST);
                    // Create object to return.
                    $return = new stdClass();
                    $return->id = (int) $discussion->id;
                    $return->course = $discussion->course;
                    $return->ouilforum = $discussion->ouilforum;
                    ...
                    $lastpost = $DB->get_record('ouilforum_posts', array('id' => $return->lastpost), '*', MUST_EXIST);
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'Forum id'),
                    'course' => new external_value(PARAM_INT, 'Course id'),
                    'ouilforum' => new external_value(PARAM_INT, 'The forum id'),
    ...
    public static function get_forum_discussion_posts($discussionid, $sortby = "created", $sortdirection = "DESC") {
        ...
        $forum = $DB->get_record('ouilforum', array('id' => $discussion->forum), '*', MUST_EXIST);
        ...
        // This require must be here, see mod/ouilforum/discuss.php.
        require_once($CFG->dirroot.'/mod/ouilforum/lib.php');
        ...
        $allposts = ouilforum_get_all_discussion_posts($discussion->id, $sort, $forumtracked);
        ...
        foreach ($allposts as $post) {
            ...
            // Function ouilforum_get_all_discussion_posts adds postread field.
            ...
            list($post->message, $post->messageformat) =
                external_format_text($post->message, $post->messageformat, $modcontext->id, 'mod_ouilforum', 'post', $post->id);
            ...
            if (!empty($post->attachment)) {
                ...
                $fs = get_file_storage();
                if ($files = $fs->get_area_files($modcontext->id, 'mod_ouilforum', 'attachment', $post->id, "filename", false)) {
                    foreach ($files as $file) {
                        $filename = $file->get_filename();
                        $fileurl = moodle_url::make_webservice_pluginfile_url(
                                        $modcontext->id, 'mod_ouilforum', 'attachment', $post->id, '/', $filename);
    ...
    public static function get_forum_discussions_paginated($forumid, $sortby = 'timemodified', $sortdirection = 'DESC',
        ...
        $alldiscussions = ouilforum_get_discussions($cm, $sort, true, -1, -1, true, $page, $perpage, OUILFORUM_POSTS_ALL_USER_GROUPS);
        ...
        if ($alldiscussions) {
            ...
            // The forum function returns the replies for all the discussions in a given forum.
            $replies = ouilforum_count_discussion_replies($forumid, $sort, -1, $page, $perpage);

            foreach ($alldiscussions as $discussion) {

                // This function checks for qanda forums.
                // Note that the ouilforum_get_discussions returns as id the post id, not the discussion id so we need to do this.
                $discussionrec = clone $discussion;
                $discussionrec->id = $discussion->discussion;
                if (!ouilforum_user_can_see_discussion($forum, $discussionrec, $modcontext)) {
                    $warning = array();
                    // Function ouilforum_get_discussions returns ouilforum_posts ids not ouilforum_discussions ones.
                    $warning['item'] = 'post';
                ...
                list($discussion->message, $discussion->messageformat) =
                    external_format_text($discussion->message, $discussion->messageformat,
                                            $modcontext->id, 'mod_ouilforum', 'post', $discussion->id);
                ...
                if (!empty($discussion->attachment)) {
                    ...
                    $fs = get_file_storage();
                    if ($files = $fs->get_area_files($modcontext->id, 'mod_ouilforum', 'attachment',
                        ...
                        foreach ($files as $file) {
                            ...
                            $discussion->attachments[] = array(
                                ...
                                'fileurl'  => file_encode_url($CFG->wwwroot.'/webservice/pluginfile.php',
                                                '/'.$modcontext->id.'/mod_ouilforum/attachment/'.$discussion->id.'/'.$filename)
                            );
                        }
                    }
                }
    ...
    public static function view_forum($forumid) {
        ...
        $forum = $DB->get_record('ouilforum', array('id' => $params['forumid']), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($forum, 'ouilforum');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        require_capability('mod/ouilforum:viewdiscussion', $context, null, true, 'noviewdiscussionspermission', 'ouilforum');
    ...
    public static function view_forum_discussion($discussionid) {
        ...
        $discussion = $DB->get_record('ouilforum_discussions', array('id' => $params['discussionid']), '*', MUST_EXIST);
    ...
    public static function add_discussion_post($postid, $subject, $message, $options = array()) {
        ...
        if (!$parent = ouilforum_get_post_full($params['postid'])) {
            throw new moodle_exception('invalidparentpostid', 'ouilforum');
        }

        if (!$discussion = $DB->get_record("ouilforum_discussions", array("id" => $parent->discussion))) {
            throw new moodle_exception('notpartofdiscussion', 'ouilforum');
        }
        ...
        if (!ouilforum_user_can_post($forum, $discussion, $USER, $cm, $course, $context)) {
            throw new moodle_exception('nopostforum', 'ouilforum');
        }
        ...
        if ($postid = ouilforum_add_new_post($post, null)) {
            ...
            ouilforum_post_subscription($settings, $forum, $discussion);
        } else {
            throw new moodle_exception('couldnotadd', 'ouilforum');
        }
    ...
    public static function add_quick_discussion_post($postid, $subject, $message, $ashtml = 0, $postlevel = 1, $options = array()) {
        ...
        if ($discussion->locked == 1) {
        	$results = array(
        			...
        			'errortype' => 'locked',
        			'post' => '',
        			'errormessage' => get_string('cannotreplylockeddiscussion', 'ouilforum'),
        			'replybutton' => ouilforum_print_post_reply_button(true)
        	);
        	if (!has_capability('mod/ouilforum:lockmessage', $context)) {
        		$results['lockicon'] = \html_writer::span('', 'ouilforum_unlock icon_status', array(
        	...
        	return $results;
        	//throw new moodle_exception('cannotreplylockeddiscussion', 'ouilforum');
        }
        ...
        if ($postid = ouilforum_add_new_post($post, null)) {
            ...
            $event = \mod_ouilforum\event\post_created::create($event_params);
            $event->add_record_snapshot('ouilforum_posts', $post);
            $event->add_record_snapshot('ouilforum_discussions', $discussion);
            $event->trigger();
        ...
        if ($params['ashtml']) {
        	$new_post = ouilforum_get_post_full($postid);
        	...
        	ob_start();
        	$forumtracked = ouilforum_tp_is_tracked($forum);
        	if ($forumtracked && ($displaymode == OUILFORUM_MODE_FLATNEWEST || $displaymode == OUILFORUM_MODE_FLATOLDEST)) {
        	...
        	if ($return_full_discussion_replies) {
        		...
        		ouilforum_print_discussion($course, $cm, $forum, $discussion, $post, $displaymode, $canreply, $canrate, null, null, $postid, true);
        	} else {
	        	echo '<div class="indent flatindent'.$indent_level.'">';
	        	ouilforum_print_post($new_post, $discussion, $forum, $cm, $course, true, true, false,
    ...
    public static function add_discussion($forumid, $subject, $message, $groupid = -1, $options = array()) {
        ...
        // Request and permission validation.
        $forum = $DB->get_record('ouilforum', array('id' => $params['forumid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($forum, 'ouilforum');
        ...
        if (!groups_get_activity_groupmode($cm)) {
            ...
            if ($groupid === -1 or empty($params['groupid'])) {
                ...
                // Here we rely in the group passed, ouilforum_user_can_post_discussion will validate the group.
                $groupid = $params['groupid'];
            }
        }

        if (!ouilforum_user_can_post_discussion($forum, $groupid, -1, $cm, $context)) {
            throw new moodle_exception('cannotcreatediscussion', 'ouilforum');
        }
        ...
        if ($discussionid = ouilforum_add_discussion($discussion)) {
    ...
    public static function recommend_post($action, $post, $discussion) {
    	...
    	if ($action == 'recommendpost') {
    		return \mod_ouilforum\post_actions::recommend_post($post);
    	} else if ($action == 'unrecommendpost') {
	    	// Remove recommendation.
    		\mod_ouilforum\post_actions::unrecommend_post($post);
	    	// Now check if the discussion has recommaned messages.
    		return \mod_ouilforum\post_actions::is_discussion_recommended($discussion);
    	}
    }
    ...
    public static function flag_post($action, $post, $discussion) {
    	...
    	if ($flag_int > 0) {
    		return \mod_ouilforum\post_actions::add_flag($post->id, $USER->id, $flag_int);
    	} else if ($flag_int == 0) {
    		\mod_ouilforum\post_actions::remove_flag($post->id, $USER->id);
    		// Now check if the discussion has flagged messages.
    		return \mod_ouilforum\post_actions::is_discussion_flagged($discussion, $USER->id);
    	}
    }
    ...
    public static function single_read($forum, $post) {
    	...
    	return ouilforum_tp_mark_post_read($USER->id, $post, $forum);
    }
    ...
    public static function multiple_read($forum, $discussionid, $posts) {
    	...
    	// Mark the post as read.
   		...
   		if (!$posts = $DB->get_records_select('ouilforum_posts', 'id '.$idsql.' AND discussion=?', $idparams)) {
   			return false;
   		}
    	require_once($CFG->dirroot.'/mod/ouilforum/lib.php');
   		$posts = array_keys($posts);
    	return ouilforum_tp_mark_posts_read($USER, $posts);
    }
    ...
    public static function subscribe_discussion($discussion, $subscribe) {
        ...
        if (!$forum = $DB->get_record('ouilforum', array('id'=>$discussion->ouilforum))) {
    		throw new moodle_exception('invalidforumid', 'ouilforum');
    	}
    	require_once($CFG->dirroot.'/mod/ouilforum/lib.php');
    	if ($subscribe == 'subscribe') {
    		...
    		if (!\mod_ouilforum\subscriptions::is_subscribable($forum)) {
    			return array(
    					...
    					'errormessage'=>get_string('errorsubscribediscussion:unsubscribable', 'ouilforum'));
    		} else {
    			...
    			if (\mod_ouilforum\subscriptions::is_subscribed($USER->id, $forum)) {
    				return array(
    						...
    						'errormessage'=>get_string('errorsubscribediscussion:subscribedtoforum', 'ouilforum'));
    			} else {
	    			if(\mod_ouilforum\subscriptions::subscribe_user_to_discussion($USER->id, $discussion)) {
    		...
    		if (\mod_ouilforum\subscriptions::unsubscribe_user_from_discussion($USER->id, $discussion)) {
    			return array('error'=>false);
    		} else {
    			return array('error'=>true);
    		}
    ...
    public static function add_quick_discussion($forumid, $subject, $message, $groupid = -1, $togroups = 0, $ashtml = 0, $options = array()) {
        ...
        self::validate_context($context);
        $groupslist = array();
        $can_post_to_groups = $params['togroups'] && ouilforum_can_post_to_groups($forum, $cm, $course, $context, $groupslist);
        // Normalize group.
        if (!groups_get_activity_groupmode($cm)) {
            // Groups not supported, force to -1.
            $groupid = -1;
        ...
        if (!$can_post_to_groups && !ouilforum_user_can_post_discussion($forum, $groupid, -1, $cm, $context)) {
            throw new moodle_exception('cannotcreatediscussion', 'ouilforum');
        }
        ...
        foreach ($groupslist as $gid) {
	        ...
	        if ($discussionid = ouilforum_add_discussion($discussion)) {
	            ...
	            $event = \mod_ouilforum\event\discussion_created::create($event_params);
	            $event->add_record_snapshot('ouilforum_discussions', $discussion);
	            $event->trigger();
        ...
        if ($params['ashtml']) {
	        ...
	        foreach ($new_ids as $newid) {
	        	ouilforum_get_single_discussion($forum, $context, $course, $cm, $newid, OUILFORUM_DISPLAY_OPEN);
	        }
    ...
    public static function delete_post($postid=0) {
    	...
    	if (!$post = ouilforum_get_post_full($postid)) {
    		throw new moodle_exception('invalidpostid', 'ouilforum');
    	}
    	...
    	$modcontext = context_module::instance($cm->id);
    	$delete_any_post = has_capability('mod/ouilforum:deleteanypost', $modcontext);
    	if ($post->userid != $USER->id && !$delete_any_post) {
    		throw new moodle_exception('cannotdeletepost', 'ouilforum');
    	}
    	...
    	if ($post->totalscore) {
    		...
    		throw new moodle_exception('couldnotdeletereplies', 'ouilforum');
    	} else {
    		// Post is a discussion topic as well, so delete discussion.
    		if (!$post->parent) {
    			if ($forum->type === 'single') {
    				throw new moodle_exception('cannotdeletesinglediscussion', 'ouilforum');
    			}
				...
				if (!$post->parent && $forum->type === 'eachuser' && $post->userid == $USER->id) {
					...
					if (ouilforum_user_can_post_discussion($forum, $currentgroup, -1, $cm, $context)) {
						$canstart = true;
						$button = ouilforum_print_new_message_button($canstart, $forum, $currentgroup, $groupmode, $context);
						$dialog = ouilforum_print_new_message_dialog($forum, $groupmode, $currentgroup);
						return array(
								'deleted'=>true,
								'button'=>$button,
								'dialog'=>$dialog
    			...
    			if (!ouilforum_delete_post($post, $delete_option, $course, $cm, $forum)) {
    				throw new moodle_exception('errorwhiledelete', 'ouilforum');
    			}
    			return array('deleted'=>true);
   			}
    	}
    ...
    public static function subscribe_forum($forumid, $subscribe) {
    	 ...
    	 if ($subscribe == 'unsubscribe') {
    	 	return \mod_ouilforum\subscriptions::unsubscribe_user($USER->id, $forum, $context, true);
    	 }
    	 else if ($subscribe == 'subscribe') {
    	 	$issubscribed = \mod_ouilforum\subscriptions::is_subscribed($USER->id, $forum, null, $cm);
    	 	if ($issubscribed) {
    	 		return true;
    	 	} else {
    	 		...
    	 		if ($groupmode && !has_capability('moodle/site:accessallgroups', $context)) {
    	 			if (!groups_get_all_groups($course->id, $USER->id)) {
    	 				throw new moodle_exception('cannotsubscribe', 'ouilforum');
    	 			}
    	 		}
    	 		if (!\mod_ouilforum\subscriptions::is_subscribable($forum)) {
    	 			throw new moodle_exception('cannotsubscribe', 'ouilforum');
    	 			...
    	 			if (!isguestuser($USER ) && (is_enrolled($context, $USER, '', true)) || has_capability('moodle/course:view', $coursecontext)) {
    	 				if (\mod_ouilforum\subscriptions::subscribe_user($USER->id, $forum) != false) {
    	 					return true;
    	 				} else {
    	 					return false;
    	 				}
    ...
    public static function subscribe_forums($courseid, $subscribe) {
    	...
    	\mod_ouilforum\subscriptions::fill_subscription_cache_for_course($courseid, $USER->id);
    	$modinfo = get_fast_modinfo($course);
    	$forums = $DB->get_records('ouilforum', array('course'=>$courseid));
    	...
    	foreach ($modinfo->get_instances_of('ouilforum') as $forumid=>$cm) {
    		if (!$cm->uservisible or !isset($forums[$forumid])) {
    	 	...
    	 	if ($subscribe == 'unsubscribe') {
    	 		\mod_ouilforum\subscriptions::unsubscribe_user($USER->id, $forums[$forumid], $context, true);
    	 		$actions[] = $forumid;
    	 	} else if ($subscribe == 'subscribe') {
    	 		if (\mod_ouilforum\subscriptions::is_subscribed($USER->id, $forums[$forumid], null, $cm)) {
    	 			$actions[] = $forumid;
    	 			continue;
    	 		} else {
    	 			if (isset($cm->groupmode) && empty($course->groupmodeforce)) {
    	 			...
    	 			if (!\mod_ouilforum\subscriptions::is_subscribable($forums[$forumid])) {
    	 				continue;
    	 			} else {
    	 				if (is_enrolled($context, $USER, '', true) || has_capability('moodle/course:view', $coursecontext)) {
    	 					if (\mod_ouilforum\subscriptions::subscribe_user($USER->id, $forums[$forumid]) != false) {
    	 						$actions[] = $forumid;
    	 						continue;
    	 					} else {
    	 						continue;
    ...
    public static function track_forum($forumid, $track, $inview, $discussions=null) {
    	...
    	$is_tracked = ouilforum_tp_is_tracked($forum, $USER);
    	...
    	if ($track == 'untrack') {
    		if (!$is_tracked) {
    			...
    			if (ouilforum_tp_stop_tracking($forum->id, $USER->id)) {
    		...
    		if (!$is_tracked) {
    			...
    			if (ouilforum_tp_start_tracking($forum->id, $USER->id)) {
    		...
    		$unread = 0;
    		if ($inview) {
    			$unread = ouilforum_tp_get_untracked_posts($USER->id, $discussions);
    		} else {
    			$unread = ouilforum_tp_count_forum_unread_posts($cm, $course);
    		}
    ...
    public static function track_forums($courseid, $track) {
    	...
    	foreach ($modinfo->get_instances_of('ouilforum') as $forumid=>$cm) {
    		...
    		$context = context_module::instance($cm->id);
    		$is_tracked = ouilforum_tp_is_tracked($forums[$forumid], $USER);
    		$eventparams = array(
    				'context' => $context,
    				'relateduserid' => $USER->id,
    		...
    		if ($track == 'untrack') {
    			if (!$is_tracked) {
    				...
    				if (ouilforum_tp_stop_tracking($forumid, $USER->id)) {
    					$event = \mod_ouilforum\event\readtracking_disabled::create($eventparams);
    					$event->trigger();
    					$actions[$forumid] = array('id'=>$forumid, 'unread'=>0);
    					continue;
    			...
    			if (!$is_tracked) {
    				if (!ouilforum_tp_can_track_forums($forums[$forumid])) {
    					continue;
    				}
    				if (ouilforum_tp_start_tracking($forumid, $USER->id)) {
    					$event = \mod_ouilforum\event\readtracking_enabled::create($eventparams);
    					$event->trigger();
    				}
    			}
    	...
    	if (!empty($tracked)) {
    		foreach ($modinfo->get_instances_of('ouilforum') as $forumid=>$cm) {
    			if (isset($tracked[$forumid])) {
    				$actions[$forumid]['unread'] = ouilforum_tp_count_forum_unread_posts($cm, $course);
    			}
    		}
    	}
        if (empty($actions)) {
    ...
    public static function lock_discussion($forumid, $discussionid, $lock) {
    	global $CFG, $DB;
    	if (!$forum = $DB->get_record("ouilforum", array("id" => $forumid))) {
    		throw new moodle_exception('invalidcoursemodule', 'ouilforum');
    	}
    	if (!$course = $DB->get_record("course", array("id" => $forum->course))) {
    		throw new moodle_exception('invalidcourseid');
    	}
    	...
    	if (!$discussion = $DB->get_record('ouilforum_discussions', array('id'=>$discussionid))) {
    		throw new moodle_exception('cannotfinddiscussion', 'ouilforum');
    	}
    	
    	$discussion->locked = $lock == 'lock' ? 1 : 0;
    	if (!$DB->update_record("ouilforum_discussions", $discussion)) {
    		throw new moodle_exception("couldnotupdate", "ouilforum");
    	}
    	require_once($CFG->dirroot.'/mod/ouilforum/lib.php');
    	$output = ouilforum_print_post_reply_button($lock == 'lock');
    	return $output;
    }
    ...
    public static function quick_forward_post($postid, $forumid, $email, $subject, $ccme=false, $message) {
    	...
    	if (!$DB->record_exists('user', array('email'=>$params['email']))) {
    		$results = array(
    				'error' => true,
    				'errortype' => 'email',
    				'errormessage' => get_string('useremaildontexist', 'ouilforum')
    		);
    		return $results;
    	}
    	if (!$post = ouilforum_get_post_full($params['postid'])) {
    		throw new moodle_exception('invalidpostid', 'ouilforum', '', $params['postid']);
    	}
    	// Request and permission validation.
    	$forum = $DB->get_record('ouilforum', array('id' => $params['forumid']), '*', MUST_EXIST);
    	$course = get_course($forum->course);
    	...
    	$msg = ouilforum_print_post_plain($post, $cm, $course, $forum, $form, get_string('forwardpreface', 'ouilforum', $a), false, $userto->timezone);
    	
    	$subject = stripslashes($params['subject']).' '.get_string('strconsubject', 'ouilforum').' '.$course_unique;
    	if (!email_to_user($userto, $USER, $subject, $msg[FORMAT_PLAIN], $msg[FORMAT_HTML])) {
    		throw new moodle_exception('errorforwardemail', 'ouilforum', '', $userto->email);
    	}
    	
    	// Send to me.
    	if (!empty($params['ccme'])) {
    		if (!email_to_user($USER, $USER, $subject, $msg[FORMAT_PLAIN], $msg[FORMAT_HTML])) {
    			throw new moodle_exception('errorforwardemail', 'ouilforum', '', $USER->email);
    		}
    	}
    ...
    public static function mark_forum_read($forumid, $discussionid=null) {
        ...
        if (!ouilforum_tp_is_tracked($forum, $USER)) {
    		return true;
    	}
    	...
    	if (!empty($discussionid)) {
    		if (!$DB->record_exists('ouilforum_discussions', array('id'=> $discussionid, 'ouilforum'=> $forumid))) {
    			throw new moodle_exception('invaliddiscussionid', 'ouilforum');
    		}
    		return ouilforum_tp_mark_discussion_read($USER, $discussionid);
    	} else {
    		...
    		return ouilforum_tp_mark_forum_read($USER, $forum->id, $currentgroup);
    	}
    }
